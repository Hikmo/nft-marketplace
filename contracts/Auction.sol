...
contract Auction is ReentrancyGuard{
    ...
    function withdrawFunds() external nonReentrant returns(bool){
        ...
        (address receiver, uint royalty) = IERC2981(nftAddress).royaltyInfo(market.getMarketItemById(itemId).tokenId, maxBid);
        admin.transfer(maxBid - royalty);
        payable(receiver).transfer(royalty);
        emit WithdrawFunds(msg.sender,maxBid);
        return true;
    }

    function getRefunds() external nonReentrant returns(bool){
        require(getAuctionState() == AuctionState.ENDED || getAuctionState() == AuctionState.DIRECT_BUY || getAuctionState() == AuctionState.CANCELLED);
