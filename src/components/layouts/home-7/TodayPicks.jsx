...
const TodayPicks = (props) => {
  ...
  const [collectionLen, setCollectionLen] = useState(0);
  ...
  useEffect(() => {
    async function fillChainValues() {
      const _collectionLen = (
        await marketContract.collectionInfos(collectionId)
      ).len.toNumber();
      console.log(_collectionLen + " : collection length");
      setCollectionLen(_collectionLen);
      const _itemIds = [];
      const datas = [];
      for (let i = 0; i < _collectionLen; i++) {
        const itemId = (
          await marketContract.getItemByCollectionId(collectionId, i)
        ).toNumber();
        _itemIds.push(itemId);
        const marketItem = await marketContract.getMarketItemById(itemId);
        console.log(marketItem);
        const nftContractAddress = marketItem.nftContract;
        ...
        const auctionAddress = marketItem.auctionAddress;
        const auctionContract = new ethers.Contract(
          auctionAddress,
          auctionData.abi,
          signer
        );
        const maxBid = (await auctionContract.maxBid()).toString();
        const data = {
          name: metaData.name,
          image: metaData.image,
          description: metaData.description,
          artist: metaData.artist,
          artistImg: metaData.artistImg ?? "",
          maxBid: maxBid,
          ...
          <div className="row">
            <div className="col-md-12">
              <div className="heading-live-auctions mg-bt-21">
                <h2 className="tf-title">All NFTs of this Collection</h2>
            ...
            <div className="content-inner">
              {nftInfos.map((item, index) => (
                <div
                  style={(nftInfos.length < 3) ? {minWidth: "45%"} : {}}
                  key={index}
                  className="sc-card-product menu_card style-h7"
                  onClick={() => {
                    navigate("/item-details-01", {
                      state: {
                  ...
                  <div className="meta-info style">
                    <div className="author">
                      <div className="avatar">
                        <img src={item.artistImg} alt="Axies" />
                      </div>
                      <div className="info">
                        <span>Creator</span>
                        <h6>
                  ...
                  <div className="meta-info">
                    <div className="author">
                      <div className="info style2">
                        ...
                        <span className="pricing">{item.maxBid}</span>
                      </div>
                    </div>
                  </div>
